...
export async function checkoutQuote(
  userId: string,
  db: Pick<ReturnType<typeof getDb>, 'cart' | 'address'> = getDb(),
) {
  ...
  const fee = 0
  if (!Number.isSafeInteger(subtotal) || subtotal + fee > 99_999_999_999_999)
    throw new CheckoutError('مبلغ سفارش بیش از حد مجاز است.')
  const shipping = {
  ...
  const quoteHash = createHash('sha256')
    .update(JSON.stringify({ items, shipping, fee }))
    .digest('hex')
  return {
